...
def main():	
	...
	kp = int(sys.argv[3])
	if(k < 0):
		print("Largeur négative")
		quit()
	if(kp < 0):
		print("Hauteur négative")
		quit()
	im = Image.open(chemin)
	if(k > im.size[1]):
		print("dépassement de la largeur de l'image")
		quit()
	if(kp > im.size[0]):
		print("dépassement de la hauteur de l'image")
		quit()
	img = gradient(im)
	compteur = k
	while compteur>0:
